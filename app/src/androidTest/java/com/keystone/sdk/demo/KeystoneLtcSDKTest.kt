package com.keystone.sdk.demo

import androidx.test.ext.junit.runners.AndroidJUnit4
import com.keystone.module.Input
import com.keystone.module.KeystoneSignRequest
import com.keystone.module.LtcTx
import com.keystone.module.Output
import com.keystone.module.UTXO
import com.keystone.sdk.KeystoneSDK
import com.sparrowwallet.hummingbird.UR
import io.mockk.every
import io.mockk.mockkObject

import org.junit.Test
import org.junit.runner.RunWith

import org.junit.Assert.*

/**
 * Instrumented test, which will execute on an Android device.
 *
 * See [testing documentation](http://d.android.com/tools/testing).
 */
@RunWith(AndroidJUnit4::class)
class KeystoneLtcSDKTest {

    @Test
    fun parseSignResult() {
        val sdk = KeystoneSDK()
        val ur = UR("keystone-sign-result", "".decodeHex())
        val signResult = sdk.ltc.parseSignResult(ur)
        assertEquals(signResult.requestId, "cc946be2-8e4c-42be-a321-56a53a8cf516")
        assertEquals(signResult.rawData, "")
    }

    @Test
    fun generateSignRequest() {
        val inputs = ArrayList<Input>()
        inputs.add(
            Input(
                "a59bcbaaae11ba5938434e2d4348243e5e392551156c4a3e88e7bdc0b2a8f663",
                2,
                UTXO(
                    "035684d200e10bc1a3e2bd7d59e58a07f2f19ef968725e18f1ed65e13396ab9466",
                    18519750
                ),
                "m/49'/2'/0'/0/0"
            )
        )
        val outputs = ArrayList<Output>()
        outputs.add(
            Output(
                "MUfnaSqZjggTrHA2raCJ9kxpP2hM6zezKw",
                10000,
            )
        )
        outputs.add(
            Output(
                "MK9aTexgpbRuMPqGpMERcjJ8hLJbAS31Nx",
                18507500,
                true,
                "M/49'/2'/0'/0/0"
            )
        )
        val signData = LtcTx(
            inputs,
            outputs,
            2250
        )

        val sdk = KeystoneSDK().ltc
        mockkObject(sdk)
        every { sdk.getTimestamp() } returns 1610467200L
        KeystoneSDK.maxFragmentLen = 400
        val res = sdk.generateSignRequest(KeystoneSignRequest(
            "cc946be2-8e4c-42be-a321-56a53a8cf516",
            signData,
            "F23F9FD2",
            "LTC Wallet"
        ))
        assertEquals(
            res.nextPart(),
            "ur:keystone-sign-"
        )
    }
}
